#!/bin/bash
#
# generate_6mer_profiles.sh
# Generate 6-mer frequency profiles for bacterial genomes using Jellyfish
#
# Usage: ./generate_6mer_profiles.sh <genome.fasta> <output_dir>
#
...
# Output files
JF_FILE="${OUTPUT_DIR}/${GENOME_ID}_6mer.jf"
DUMP_FILE="${OUTPUT_DIR}/${GENOME_ID}_6mer.txt"
STATS_FILE="${OUTPUT_DIR}/${GENOME_ID}_6mer_stats.txt"
...
# - For 6-mers, there are 4^6 = 4096 possible k-mers (2048 canonical)
echo "Counting 6-mers..."
jellyfish count \
    -m 6 \
    -s 100M \
    -t 4 \
...
echo "Generating statistics..."
{
    ...
    echo "Date: $(date)"
    echo "---"
    echo "Total unique 6-mers:"
    wc -l < "$DUMP_FILE"
    echo ""
    echo "Total 6-mer observations:"
    awk '{sum+=$2} END {print sum}' "$DUMP_FILE"
    echo ""
    echo "Most frequent 6-mers (top 10):"
    sort -k2,2nr "$DUMP_FILE" | head -10
    echo ""
    echo "Least frequent 6-mers (bottom 10):"
